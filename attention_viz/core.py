...
from .visualizers import AttentionHeatmap, AttentionFlow, LayerEvolution
...
class AttentionVisualizer:
    ...
    def __init__(
        self,
        model: Union[CLIPModel, torch.nn.Module],
    ...
    def visualize(
        ...
        **kwargs
        ...
        if visualization_type == "heatmap":
            return self.heatmap_viz.create(
                attention_data,
                inputs,
                **kwargs
            )
        elif visualization_type == "flow":
            return self.flow_viz.create(
                attention_data,
                inputs,
                **kwargs
            )
        elif visualization_type == "evolution":
            return self.evolution_viz.create(
                attention_data,
                inputs,
                **kwargs
            )
    ...
    def _get_top_attended_tokens(
        ...
        top_tokens = [(tokens[i] if i < len(tokens) else f"token_{idx}", 
                      avg_attention[idx]) for i, idx in enumerate(top_indices)]
